...
(async function(){
  ...
  const DATA_V = '__DATA_V__';   // the build's fingerprint: data addresses change when the data does
  ...
  function gridRows(){ return 150; }                       // the fine grid, on every screen — the grain is the look
  function fineBlur(){ return 0; }
  function fineBoost(){ return 1; }
  // On a phone or a laptop the dots are too small for the map's thin lines to survive the dot-screen. Rather
  // than coarsen the grid, the coast, ridges and roads are laid over the finished bake as hairlines — the
  // same weight as the HUD geometry on the other pages — dotted so they still read as part of the screen.
  // Strength fades to nothing as the dots grow: a big screen shows exactly what it showed before.
  function hairlineStrength(pitch){ return Math.max(0, Math.min(1, (6.2 - pitch) / 3.2)); }
  function overlayLines(g, W, H, box, pitch){
    const s = hairlineStrength(pitch); if(!GEO || s <= 0) return;
    const P = (lon, lat) => [ (lon - box.lon0)/(box.lon1 - box.lon0) * W, (lat - box.lat0)/(box.lat1 - box.lat0) * H ];
    const stroke = (pts, alpha, width, dash) => {
      g.strokeStyle = `rgba(255,255,255,${(alpha * s).toFixed(3)})`; g.lineWidth = width; g.setLineDash(dash);
      g.beginPath(); pts.forEach((p, i) => { const q = P(p[0], p[1]); i ? g.lineTo(q[0], q[1]) : g.moveTo(q[0], q[1]); }); g.stroke();
    };
    g.save(); g.globalCompositeOperation = 'lighter'; g.lineCap = 'round'; g.lineJoin = 'round';
    GEO.ridges.forEach(r => stroke(r, 0.55, 0.8, [1, 2.6]));
    GEO.roads.forEach(r => stroke(r, 0.5, 0.7, [2.4, 2.4]));
    stroke(GEO.coast, 0.9, 1.1, [1.6, 2.2]);
  ...
  (function(){
    ...
    function bakeMap(){
      ...
      mctx.setTransform(dpr, 0, 0, dpr, 0, 0); overlayLines(mctx, cw, ch, CHART_BOX, pitch); mctx.setTransform(1, 0, 0, 1, 0, 0);
    }
  ...
  function bakeMini(R){
    ...
    const ny = gridRows(ch), nx = Math.round(ny * cw / ch), pitch = cw / nx;
    ...
    x.setTransform(dpr, 0, 0, dpr, 0, 0); overlayLines(x, cw, ch, tacticalBox, pitch);
    if(hairlineStrength(pitch) > 0){   // the same edge fade the drawing has, so the hairlines dissolve at the frame rather than run to it
      const fh = x.createLinearGradient(0,0,cw,0); fh.addColorStop(0,'rgba(0,0,0,.9)'); fh.addColorStop(.12,'rgba(0,0,0,0)'); fh.addColorStop(.88,'rgba(0,0,0,0)'); fh.addColorStop(1,'rgba(0,0,0,.9)');
      const fv = x.createLinearGradient(0,0,0,ch); fv.addColorStop(0,'rgba(0,0,0,.9)'); fv.addColorStop(.1,'rgba(0,0,0,0)'); fv.addColorStop(.9,'rgba(0,0,0,0)'); fv.addColorStop(1,'rgba(0,0,0,.9)');
      x.fillStyle = fh; x.fillRect(0,0,cw,ch); x.fillStyle = fv; x.fillRect(0,0,cw,ch);
    }
